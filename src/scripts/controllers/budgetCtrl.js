  ...
  .controller(
    ...
    function (
      ...
      this.addCategory = (name, masterCategory) => {
        let sort = masterCategory.categories[
          masterCategory.categories.length - 1
        ]
          ? masterCategory.categories[masterCategory.categories.length - 1]
              .sort + 1
          : 0;
      ...
      this.removeCategory = (id) => {
        removeConfirm($scope.dbCtrl.categories[id].name).then(() => {
          return remove();
        });
      ...
      this.removeMasterCategory = (masterCategory) => {
        removeConfirm(masterCategory.name).then(() => {
          return remove();
        });
